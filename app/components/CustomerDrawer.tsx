...
export default function CustomerDrawer({
  ...
  const hasNext =
    currentIndex !== -1 && currentIndex < navigationCustomers.length - 1;
  ...
  const handlePrevious = () => {
    ...
    const previousCustomer = navigationCustomers[currentIndex - 1];

    if (previousCustomer) {
      onNavigate(Number(previousCustomer.id));
    }
  };
  ...
  const handleNext = () => {
    ...
    const nextCustomer = navigationCustomers[currentIndex + 1];

    if (nextCustomer) {
      onNavigate(Number(nextCustomer.id));
    }
  };
  ...
  const formattedDeadline = customer.deadline
    ? new Date(`${customer.deadline}T00:00:00`).toLocaleDateString("en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  const getDeadlineCountdown = () => {
    ...
    const deadlineDate = new Date(`${customer.deadline}T00:00:00`);
    ...
    if (daysUntil < 0) {
      ...
      return {
        text: `${daysAgo} ${daysAgo === 1 ? "day" : "days"} past deadline`,
        className: "font-bold text-[#424A52]",
      };
    }
    ...
    if (daysUntil === 0) {
      return {
        ...
        className: "font-bold text-[#C21F35]",
      };
    }
    ...
    if (daysUntil <= 30) {
      return {
        ...
        className: "font-bold text-[#A84F0F]",
      };
    }

    return {
      text: `${daysUntil} days remaining`,
      className: "font-bold text-[#216B45]",
    };
  };
  ...
  const getAttachmentFileName = (fileName: string | null, filePath: string) => {
    if (fileName?.trim()) {
  ...
  const formatBooleanValue = (value: string | null | undefined) => {
  ...
  const limitedOpportunity = formatBooleanValue(customer.limited_opportunity);

  const fellowshipOpportunity = formatBooleanValue(
  ...
  const getCategoryStyle = (category: string) => {
    ...
    const matchingCategory = Object.keys(categoryColorMap).find(
      (key) => key.trim().toLowerCase() === normalizedCategory,
    );

    if (matchingCategory) {
      return categoryColorMap[matchingCategory];
    }
    ...
    const categoryIndex = availableCategories.findIndex(
      (availableCategory) =>
        availableCategory.trim().toLowerCase() === normalizedCategory,
    );
  ...
  const recordTotal = Math.max(navigationCustomers.length, 1);
            ...
            <div className="min-w-0 pr-1 sm:pr-4">
              <div className="min-w-0">
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#D4D9DC]">
                  {isProspect
                    ? "Prospect Library"
                    : customer.grantor || "Grant Opportunity"}
                </p>
                <h2
                ...
                >
                  {isProspect
                    ? customer.grantor_name ||
                      customer.grantor ||
                      "Unnamed Grantor"
                    : customer.opportunity_name || "Untitled Opportunity"}
                </h2>
              </div>
            </div>
              ...
              <>
                      ...
                      <p className="mt-2 text-xl font-bold text-[#2F3038]">
                        {customer.grant_minimum
                          ? new Intl.NumberFormat("en-US", {
                            ...
                            }).format(Number(customer.grant_minimum))
                          : "Not specified"}
                      </p>
                    </div>
                    <div className="rounded-2xl border border-[#C8CBCC] bg-white p-5 shadow-[0_8px_24px_rgba(47,48,56,0.06)]">
                      ...
                      <p className="mt-2 text-xl font-bold text-[#2F3038]">
                        {customer.grant_maximum
                          ? new Intl.NumberFormat("en-US", {
                              style: "currency",
                              currency: "USD",
                              maximumFractionDigits: 0,
                            }).format(Number(customer.grant_maximum))
                          : "Not specified"}
                      </p>
                    </div>
                    <div className="rounded-2xl border border-[#C8CBCC] bg-[#E9E9E7] p-5 sm:col-span-2">
                      <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#778189]">
                        RFP Cycle
                      </p>
                      <p className="mt-2 text-sm font-semibold leading-6 text-[#394147]">
                        {customer.rfp_cycle || "Not documented"}
                      </p>
                    </div>
                  </div>
                </section>
                ...
                <section className="rounded-[22px] border border-[#C8CBCC] bg-[#E9E9E7] p-6">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2.5">
                    {(customer.categories ?? customer.rfp_categories ?? [])
                      .length ? (
                      (
                        customer.categories ??
                        ...
                        <span
                          key={category}
                          className={`inline-flex rounded-full border px-4 py-2 text-sm font-semibold shadow-sm ${getCategoryStyle(category)}`}
                        >
                          {category}
                        </span>
                      ))
                ...
                <section className="rounded-[22px] border border-[#C8CBCC] bg-white p-6 shadow-[0_10px_28px_rgba(47,48,56,0.06)]">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2.5">
                    {(customer.program_areas ?? []).length ? (
                      (customer.program_areas ?? []).map((area) => (
                        <span
                          key={area}
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {contact?.name && (
                          <button
                            ...
                            className="rounded-lg border border-[#C8CBCC] bg-white px-2.5 py-1 text-[10px] font-bold text-[#626A70]"
                          >
                    ...
                    <div className="rounded-2xl border border-[#D7D9DA] bg-[#F4F3F1] p-4">
                      ...
                      {contact?.email ? (
                        <a
                          ...
                          className="mt-2 block break-all text-sm font-semibold text-[#8D4D45] underline underline-offset-4"
                        >
                          {contact.email}
                        </a>
                      ) : (
            ...
            {!isProspect && (
              <>
                ...
                <section aria-labelledby="snapshot-heading">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="rounded-2xl border border-[#C8CBCC] bg-[#E9E9E7] p-5">
                      ...
                      <p
                        className={`mt-2 text-sm ${deadlineCountdown.className}`}
                      >
                ...
                <section className="rounded-[22px] border border-[#C8CBCC] bg-white p-6 shadow-[0_10px_28px_rgba(47,48,56,0.06)]">
                  ...
                  <div className="mt-4 whitespace-pre-line text-[15px] leading-7 text-[#565E64]">
                    {customer.abstract || "No abstract provided."}
                  </div>
                </section>
                ...
                <section className="rounded-[22px] border border-[#C8CBCC] bg-[#E9E9E7] p-5 sm:p-6">
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="flex items-center justify-between gap-4 rounded-2xl border border-[#C8CBCC] bg-white p-4">
                      ...
                      >
                        {limitedOpportunity || "Not specified"}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-4 rounded-2xl border border-[#C8CBCC] bg-white p-4">
                      ...
                      >
                        {fellowshipOpportunity || "Not specified"}
                      </span>
                    </div>
                  </div>
                </section>
