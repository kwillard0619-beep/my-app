...
import { createCategoryColorMap, getCategoryStyle } from "./categoryColors";
import { getAnticipatedDeadlineStyle } from "./anticipatedDeadlineColors";
...
type SortField =
  "grantor" | "maximum_grant" | "deadline" | "anticipated_deadline";
...
export default function CustomerTable({
  ...
  customers: Customer[];
  activeCount: number;
  mode?: "active" | "archived" | "favorites" | "prospects";
}) {
  ...
  const [favoriteView, setFavoriteView] = useState<"active" | "archived">(
    "active",
  );

  const isArchivedView =
    mode === "archived" ||
    (mode === "favorites" && favoriteView === "archived");

  const isProspectView = mode === "prospects";

  const prospectCategoryCoverage = useMemo(() => {
    const counts = new Map<string, number>();

    customers.forEach((customer) => {
      (customer.categories ?? customer.rfp_categories ?? []).forEach(
        (category) => {
  ...
  const largestProspect = useMemo(
    () =>
      [...customers]
        .filter((customer) => Number.isFinite(Number(customer.grant_maximum)))
        .sort(
          (first, second) =>
            Number(second.grant_maximum) - Number(first.grant_maximum),
        )[0] ?? null,
    [customers],
  );

  const recentlyUpdatedProspects = useMemo(
    () =>
      [...customers]
        .sort(
          (first, second) =>
  ...
  const favoriteActiveCount = useMemo(
    () =>
      customers.filter(
        (customer) => String(customer.status).trim().toLowerCase() === "active",
      ).length,
    [customers],
  );
  ...
  const favoriteArchivedCount = useMemo(
    () =>
      customers.filter(
        (customer) =>
          String(customer.status).trim().toLowerCase() === "archived",
      ).length,
    [customers],
  );
  ...
  useEffect(() => {
    ...
    async function loadFavorites() {
      ...
      if (error) {
        ...
        setFavoriteIds(
          new Set(
            favoriteRows.map((favorite) => String(favorite.opportunity_id)),
          ),
        );
      }
  ...
  const toggleFavorite = async (opportunityId: Customer["id"]) => {
  ...
  useEffect(() => {
    ...
    const selectedRow = rowRefs.current[String(selectedCustomerId)];
  ...
  const [sortRules, setSortRules] = useState<SortRule[]>(() =>
    mode === "prospects"
      ? [
          { id: 1, field: "grantor", direction: "asc" },
          { id: 2, field: "maximum_grant", direction: "desc" },
        ]
      : [
          { id: 1, field: "deadline", direction: "asc" },
          { id: 2, field: "anticipated_deadline", direction: "asc" },
          { id: 3, field: "maximum_grant", direction: "desc" },
        ],
  );

  const [nextSortId, setNextSortId] = useState(4);

  const [draggedSortId, setDraggedSortId] = useState<number | null>(null);

  const [dragOverSortId, setDragOverSortId] = useState<number | null>(null);
  ...
  const quickFilterButtonRef = useRef<HTMLButtonElement | null>(null);

  const quickFilterPopoverRef = useRef<HTMLDivElement | null>(null);

  const [quickGrantors, setQuickGrantors] = useState<string[]>([]);

  const [quickMaximumGrants, setQuickMaximumGrants] = useState<string[]>([]);

  const [quickDeadline, setQuickDeadline] = useState("all");

  const [quickMonths, setQuickMonths] = useState<string[]>([]);
  ...
  const advancedFilterButtonRef = useRef<HTMLButtonElement | null>(null);

  const advancedFilterPopoverRef = useRef<HTMLDivElement | null>(null);
  ...
  const [openAdvancedDropdownId, setOpenAdvancedDropdownId] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      ...
      if (
        ...
        !quickFilterPopoverRef.current.contains(target) &&
        !quickFilterButtonRef.current?.contains(target)
      ) {
      ...
      if (
        ...
        !advancedFilterPopoverRef.current.contains(target) &&
        !advancedFilterButtonRef.current?.contains(target)
      ) {
  ...
  const availableCategories = useMemo(() => {
    const categories = customers.flatMap((customer) =>
      Array.isArray(customer.rfp_categories) ? customer.rfp_categories : [],
    );

    return Array.from(
      new Set(
        categories.filter(Boolean).map((category) => String(category).trim()),
      ),
  ...
  const categoryColorMap = useMemo(() => {
    return createCategoryColorMap(availableCategories);
  }, [availableCategories]);
  ...
  const availableGrantors = useMemo(() => {
    return Array.from(
      new Set(
        customers
          .map((customer) => String(customer.grantor ?? "").trim())
          .filter(Boolean),
      ),
    ).sort((a, b) => a.localeCompare(b));
  }, [customers]);
  ...
  const matchesMaximumGrantRange = (
    ...
    const amount = parseMaximumGrant(value);
    const range = MAXIMUM_GRANT_RANGES.find(
      (candidate) => candidate.label === rangeLabel,
    );
  ...
  const availableMaximumGrants = useMemo(() => {
    return MAXIMUM_GRANT_RANGES.filter((range) =>
      customers.some((customer) =>
        matchesMaximumGrantRange(customer.maximum_grant, range.label),
      ),
    )
  ...
  const availableMonths = useMemo(() => {
    const months = customers
      .map((customer) => String(customer.anticipated_deadline ?? "").trim())
      .filter(Boolean);
    ...
    return Array.from(new Set(months)).sort((a, b) => {
      const aIndex = monthOrder.indexOf(a.toLowerCase());

      const bIndex = monthOrder.indexOf(b.toLowerCase());
  ...
  const availableDeadlines = useMemo((): string[] => {
    return Array.from(
      new Set(
        customers
          .map((customer) =>
            customer.deadline ? String(customer.deadline).trim() : "",
          )
  ...
  const availableLimitedOpportunities = useMemo(() => {
    return Array.from(
      new Set(
        customers
          .map((customer) => String(customer.limited_opportunity ?? "").trim())
          .filter(Boolean),
      ),
    ).sort((a, b) => a.localeCompare(b));
  }, [customers]);

  const availableFellowshipOpportunities = useMemo(() => {
    return Array.from(
      new Set(
        customers
          .map((customer) =>
            String(customer.fellowship_opportunity ?? "").trim(),
          )
          .filter(Boolean),
      ),
    ).sort((a, b) => a.localeCompare(b));
  ...
  const matchesQuickDeadline = (deadline: string | null) => {
    if (quickDeadline === "all") {
      return true;
    }

    if (quickDeadline === "no-deadline") {
      return !deadline;
    }
    ...
    const daysUntil = getDeadlineDays(deadline);

    if (daysUntil === null) {
      return false;
    }

    const relevantDays = isArchivedView ? Math.abs(daysUntil) : daysUntil;
  ...
  const getFilterValues = (
    ...
    switch (field) {
      case "grantor":
        return customer.grantor ? [String(customer.grantor)] : [];

      case "maximum_grant":
        return customer.maximum_grant ? [String(customer.maximum_grant)] : [];

      case "deadline":
        return customer.deadline ? [String(customer.deadline)] : [];

      case "anticipated_deadline":
        return customer.anticipated_deadline
          ? [String(customer.anticipated_deadline)]
          : [];

      case "category":
        return Array.isArray(customer.rfp_categories)
          ? customer.rfp_categories.map((value) => String(value))
          : [];

      case "limited_opportunity":
        return customer.limited_opportunity
          ? [String(customer.limited_opportunity)]
          : [];
  ...
  const matchesAdvancedFilter = (
    ...
    const customerValues = getFilterValues(customer, filter.field).map(
      (value) => String(value).trim().toLowerCase(),
    );

    const selectedValues = filter.values.map((value) =>
      String(value).trim().toLowerCase(),
    );

    const hasMatch = selectedValues.some((selectedValue) =>
      customerValues.includes(selectedValue),
    );

    if (filter.operator === "contains" || filter.operator === "not_contains") {
      const containsMatch = selectedValues.some((selectedValue) =>
        customerValues.some((customerValue) =>
          customerValue.includes(selectedValue),
        ),
      );
    ...
    if (
      ...
      const customerDeadline = customer.deadline
        ? new Date(`${customer.deadline}T00:00:00`).getTime()
        : null;
      ...
      return filter.values.some((value) => {
        const comparisonDeadline = new Date(`${value}T00:00:00`).getTime();

        return filter.operator === "before"
          ? customerDeadline < comparisonDeadline
          : customerDeadline > comparisonDeadline;
      });
    }
  ...
  const getAnticipatedMonthRank = (value: string | null | undefined) => {
    if (!value || !value.trim()) {
      return 999;
    }
    ...
    return monthOrder[normalized] ?? 14;
  };
  ...
  const compareDeadlines = (
    ...
    const aBlank = !a || !a.trim();

    const bBlank = !b || !b.trim();
  ...
  const compareAnticipatedDeadlines = (
    ...
    const aRank = getAnticipatedMonthRank(a);

    const bRank = getAnticipatedMonthRank(b);
  ...
  const compareMaximumGrants = (
    ...
    const aValue = parseMaximumGrant(a);

    const bValue = parseMaximumGrant(b);
  ...
  const compareBySortRule = (a: Customer, b: Customer, rule: SortRule) => {
    const descending = rule.direction === "desc";

    switch (rule.field) {
      case "grantor":
        return compareGrantors(a.grantor, b.grantor, descending);
      ...
      case "deadline":
        return compareDeadlines(a.deadline, b.deadline, descending);
  ...
  const filteredCustomers = useMemo(() => {
    const desiredStatus = mode === "favorites" ? favoriteView : mode;

    let result = isProspectView
      ? [...customers]
      : customers.filter(
          (customer) =>
            String(customer.status).trim().toLowerCase() === desiredStatus,
        );
    ...
    if (search.trim()) {
      ...
      const fuzzyMatch = (text: string, term: string) => {
        ...
        return words.some((word) => {
          ...
          const allowedDistance =
            term.length <= 4 ? 1 : term.length <= 8 ? 2 : 3;
      ...
      result = result.filter((customer) => {
        ...
        return searchableFields.some((value) =>
          fuzzyMatch(String(value ?? ""), searchTerm),
        );
      });
    }
    ...
    if (quickGrantors.length > 0) {
      result = result.filter(
        (customer) =>
          customer.grantor && quickGrantors.includes(customer.grantor),
      );
    }

    if (quickMaximumGrants.length > 0) {
      result = result.filter((customer) =>
        quickMaximumGrants.some((rangeLabel) =>
          matchesMaximumGrantRange(customer.maximum_grant, rangeLabel),
        ),
      );
    }
    ...
    if (quickDeadline !== "all") {
      result = result.filter((customer) =>
        matchesQuickDeadline(customer.deadline),
      );
    }
    ...
    if (quickCategories.length > 0) {
      result = result.filter(
        (customer) =>
          ...
          quickCategories.some((selectedCategory) =>
    ...
    if (advancedFilters.length > 0) {
      const activeRules = advancedFilters.filter(
        (filter) => filter.values.length > 0,
      );

      if (activeRules.length > 0) {
        result = result.filter((customer) => {
          let matches = matchesAdvancedFilter(customer, activeRules[0]);

          for (let index = 1; index < activeRules.length; index++) {
            const rule = activeRules[index];
            const ruleMatches = matchesAdvancedFilter(customer, rule);
    ...
    result.sort((a, b) => {
      for (const rule of sortRules) {
        const comparison = compareBySortRule(a, b, rule);
  ...
  const exportOpportunitiesToCsv = () => {
    ...
    if (isProspectView) {
      ...
      const rows = filteredCustomers.map((prospect) => [
        prospect.grantor_name ?? prospect.grantor,
        prospect.overview ?? prospect.abstract,
        (prospect.categories ?? prospect.rfp_categories ?? []).join("; "),
        (prospect.program_areas ?? []).join("; "),
      ...
      downloadLink.download = `prospect-library-${new Date()
        .toISOString()
        .slice(0, 10)}.csv`;
    ...
    const getDeadlineExportStatus = (deadline: string | null | undefined) => {
      if (!deadline) {
      ...
      if (daysUntil < 0) {
        ...
        return `${daysAgo} ${daysAgo === 1 ? "day" : "days"} past deadline`;
      }
      ...
      return `${daysUntil} ${daysUntil === 1 ? "day" : "days"} remaining`;
    };
    ...
    const headers = [
      ...
      "Categories",
      "Website",
      isArchivedView ? "Time Since Deadline" : "Deadline Countdown",
      "Abstract",
      "Contact Name",
      "Contact Email",
      "Contact Organization",
  ...
  const getQuickFacetCustomers = (excludedFacet: QuickFilterFacet) => {
    return customers.filter((customer) => {
      if (
        String(customer.status).trim().toLowerCase() !==
        (mode === "favorites" ? favoriteView : mode)
      ) {
        return false;
      }
      ...
      if (
        excludedFacet !== "anticipated_deadline" &&
  ...
  const maximumGrantFacetCustomers = getQuickFacetCustomers("maximum_grant");
  const monthFacetCustomers = getQuickFacetCustomers("anticipated_deadline");
  const categoryFacetCustomers = getQuickFacetCustomers("category");

  const pinSelectedFirst = (values: string[], selectedValues: string[]) =>
    [...values].sort((a, b) => {
  ...
  const conditionalMaximumGrantRanges = availableMaximumGrants;

  const conditionalMonths = availableMonths;

  const conditionalCategories = pinSelectedFirst(
    availableCategories,
    quickCategories,
  );

  const getMaximumGrantFacetCount = (rangeLabel: string) =>
    maximumGrantFacetCustomers.filter((customer) =>
      matchesMaximumGrantRange(customer.maximum_grant, rangeLabel),
    ).length;

  const getMonthFacetCount = (month: string) =>
    monthFacetCustomers.filter(
      (customer) =>
        customer.anticipated_deadline?.toLowerCase() === month.toLowerCase(),
    ).length;

  const getCategoryFacetCount = (category: string) =>
    categoryFacetCustomers.filter((customer) =>
      customer.rfp_categories?.some(
        (customerCategory) =>
          customerCategory.trim().toLowerCase() ===
          category.trim().toLowerCase(),
      ),
    ).length;
  ...
  const addSortRule = () => {
    ...
    const availableField = (Object.keys(SORT_FIELD_LABELS) as SortField[]).find(
    ...
    setSortRules((current) => [
      ...current,
      {
        ...
        direction: availableField === "maximum_grant" ? "desc" : "asc",
      },
    ]);

    setNextSortId((current) => current + 1);
  };

  const updateSortRule = (id: number, updates: Partial<SortRule>) => {
    setSortRules((current) =>
      current.map((rule) =>
        rule.id === id
          ? {
  ...
  const removeSortRule = (id: number) => {
  ...
  const handleSortDragOver = (
    ...
    if (draggedSortId !== null && draggedSortId !== id) {
  ...
  const resetSortRules = () => {
    ...
    setNextSortId(isProspectView ? 3 : 4);
  };
  ...
  const getSortDirectionLabel = (
    ...
    if (field === "maximum_grant") {
      return direction === "desc" ? "Highest to lowest" : "Lowest to highest";
    }

    return direction === "asc" ? "Earliest to latest" : "Latest to earliest";
  };
  ...
  const getDeadlineStyle = (deadline: string | null) => {
    ...
    if (daysUntil <= 30) {
      return {
        container: "bg-amber-100 text-amber-800 border-amber-300",
      };
    }
  ...
  const getMonthStyle = getAnticipatedDeadlineStyle;
  ...
  const removeAdvancedFilter = (id: number) => {
    setAdvancedFilters((current) =>
  ...
  const removeAdvancedFilterValue = (filterId: number, value: string) => {
    setAdvancedFilters((current) =>
      current.map((filter) =>
        filter.id === filterId
          ? {
  ...
  const activateQuickFilter = (callback: () => void) => {
    if (advancedFilters.length > 0) {
  ...
  const addMonthQuickFilter = (month: string) => {
    activateQuickFilter(() => {
  ...
  const addCategoryQuickFilter = (category: string) => {
    activateQuickFilter(() => {
  ...
  const toggleQuickFilters = () => {
    ...
    setShowQuickFilters((current) => !current);
  };
  ...
  const totalActiveFilterCount =
    activeQuickFilterCount + activeAdvancedFilterCount;

  // --------------------------------------------------
  // Advanced Filter Options
  // --------------------------------------------------

  const getAdvancedFilterOptions = (field: FilterField): string[] => {
    switch (field) {
      case "grantor":
      ...
      filteredCustomers
        .filter(
          (customer) =>
            ...
            !Number.isNaN(new Date(`${customer.deadline}T00:00:00`).getTime()),
        )
        .sort(
          (first, second) =>
            new Date(`${first.deadline}T00:00:00`).getTime() -
            new Date(`${second.deadline}T00:00:00`).getTime(),
        ),
    [filteredCustomers],
  );
  ...
  const deadlinesByDate = useMemo(() => {
    ...
    deadlineCustomers.forEach((customer) => {
      ...
      const existing = grouped.get(customer.deadline) ?? [];

      grouped.set(customer.deadline, [...existing, customer]);
    });
  ...
  const upcomingDeadlines = useMemo(() => {
    ...
    return deadlineCustomers
      .filter((customer) => new Date(`${customer.deadline}T00:00:00`) >= today)
      .slice(0, 5);
  }, [deadlineCustomers]);
  ...
  const highlightedDeadlines = useMemo(
    () =>
      mode === "favorites" && favoriteView === "archived"
        ...
        : upcomingDeadlines,
    [mode, favoriteView, recentlyClosedDeadlines, upcomingDeadlines],
  );

  useEffect(() => {
    if (mode !== "favorites") return;

    const highlightedDeadline = highlightedDeadlines[0]?.deadline;

    if (!highlightedDeadline) return;

    const targetDate = new Date(`${highlightedDeadline}T00:00:00`);
  ...
  const closingSoonCustomers = useMemo(() => {
    ...
    return deadlineCustomers.filter((customer) => {
      const deadline = new Date(`${customer.deadline}T00:00:00`);

      return deadline >= today && deadline <= thirtyDaysFromToday;
    });
  }, [deadlineCustomers]);
  ...
  const recentlyAddedCustomers = useMemo(
    () =>
      [...filteredCustomers]
        .sort(
          (first, second) =>
            (Date.parse(String(second.created_at ?? "")) || 0) -
            (Date.parse(String(first.created_at ?? "")) || 0),
        )
        .slice(0, 3),
    [filteredCustomers],
  );
  ...
  const largestClosedGrant = useMemo(() => {
    ...
    const getGrantValue = (value: Customer["maximum_grant"]) => {
      if (!value) return 0;
  ...
  const getDeadlineAccent = (customerId: Customer["id"]) => {
    const displayedIndex = highlightedDeadlines.findIndex(
      (customer) => String(customer.id) === String(customerId),
    );

    if (displayedIndex >= 0) {
      return DEADLINE_ACCENTS[displayedIndex % DEADLINE_ACCENTS.length];
    }
    ...
    <div className="min-h-screen bg-[#D4D5D6] text-[#2F3038]">
                  ...
                  <div className="min-w-0 pr-0 sm:pr-52">
                    <div className="max-w-3xl">
                      ...
                      <p className="mt-3 max-w-2xl text-sm leading-6 text-[#E9E9E7] sm:text-base">
                        {mode === "prospects"
                          ? "Research potential funders, compare program interests, and understand giving patterns before the next opportunity opens."
                          : mode === "favorites"
            ...
            {mode === "favorites" && (
              <section className="mb-7 flex flex-col gap-4 rounded-[24px] border border-[#C8CBCC] bg-[#E9E9E7] p-5 shadow-[0_10px_28px_rgba(47,48,56,0.06)] sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="inline-flex w-full rounded-2xl border border-[#C8CBCC] bg-white p-1.5 sm:w-auto">
                  <button
                    type="button"
                    onClick={() => setFavoriteView("active")}
                  ...
                  <button
                    type="button"
                    onClick={() => setFavoriteView("archived")}
                    className={`flex flex-1 items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold transition sm:flex-none ${
      ...
      ================================================== */}
            ...
            {mode === "favorites" ? (
                  ...
                  <div className="group relative overflow-hidden rounded-[26px] border border-[#C8CBCC] bg-white p-5 shadow-[0_14px_36px_rgba(63,91,108,0.08)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_44px_rgba(63,91,108,0.12)] sm:p-6">
                    ...
                    >
                      {calendarDays.map((day, index) => {
                        ...
                        const hasDeadline = dayOpportunities.length > 0;
                        const today = new Date();
                        const isToday =
                                ...
                                {dayOpportunities
                                  ...
                                  .map((opportunity) => (
                                    <span
                                      ...
                                      style={{
                                        backgroundColor: getDeadlineAccent(
                                          opportunity.id,
                                        ),
                                      }}
                                    />
                    ...
                    <div className="relative mt-4 flex flex-col gap-3 border-t border-[#D7D9DA] pt-4 text-xs text-[#626A70] sm:flex-row sm:items-center sm:justify-between">
                      <span className="flex items-start gap-2 sm:items-center">
                        <span className="flex -space-x-1">
                          {highlightedDeadlines.slice(0, 5).map((customer) => (
                            <span
                              key={customer.id}
                              className="h-2.5 w-2.5 rounded-full border border-white"
                              style={{
                                backgroundColor: getDeadlineAccent(customer.id),
                              }}
                            />
                          ))}
                        </span>
                  ...
                  <div className="relative overflow-hidden rounded-[26px] bg-[#2F3038] p-5 text-white shadow-[0_16px_40px_rgba(38,59,73,0.16)] sm:p-6">
                    ...
                    <div className="relative mt-5 space-y-2">
                      {highlightedDeadlines.length > 0 ? (
                        highlightedDeadlines.map((customer) => {
                          ...
                          return (
                            <button
                              ...
                              style={{
                                borderLeftColor: getDeadlineAccent(customer.id),
                              }}
                            >
                              <div
                              ...
                              <div className="min-w-0 flex-1">
                                <p className="flex items-center gap-2 truncate text-sm font-bold text-white">
                                  ...
                                  />
                                  {customer.grantor || "Grantor not specified"}
                                </p>
              ...
              <section className="mb-7">
                <div className="mb-5">
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#626A70]">
                    Review recently passed deadlines and notable funding
                    opportunities from the archive.
                  </p>
                </div>
                  ...
                  <div className="group relative min-h-[210px] overflow-hidden rounded-[26px] bg-[#2F3038] p-6 text-white shadow-[0_16px_40px_rgba(47,48,56,0.16)]">
                    ...
                    <div className="relative flex h-full flex-col">
                      ...
                      <p className="mt-auto pt-4 text-xs leading-5 text-[#BFC5C8]">
                        Opportunities with deadlines that passed during the last
                        30 days.
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="relative flex h-full flex-col">
                      ...
                      {largestClosedGrant ? (
                        <button
                          type="button"
                          onClick={() =>
                            setSelectedCustomerId(String(largestClosedGrant.id))
                          }
              ...
              <section className="mb-7">
                <div className="mb-5">
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#7B8791]">
                    Prospect funding landscape
                  </p>
                  <h2 className="mt-1 text-2xl font-bold tracking-[-0.025em] text-[#2F3038]">
                    Prospect Pulse
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-[#626A70]">
                    Compare giving capacity, leading funding interests, and
                    recently updated prospect research.
                  </p>
                </div>
                  ...
                  <div className="group relative min-h-[210px] overflow-hidden rounded-[26px] bg-[#2F3038] p-6 text-white shadow-[0_16px_40px_rgba(47,48,56,0.16)]">
                    ...
                    <div className="relative flex h-full flex-col">
                      ...
                      <p className="mt-1 text-3xl font-bold tracking-[-0.05em]">
                        {largestProspect?.grant_maximum
                          ? new Intl.NumberFormat("en-US", {
                              style: "currency",
                              currency: "USD",
                              maximumFractionDigits: 0,
                            }).format(Number(largestProspect.grant_maximum))
                          : "—"}
                      </p>
                      <button
                        type="button"
                        disabled={!largestProspect}
                        onClick={() =>
                          largestProspect &&
                      ...
                      >
                        <span className="line-clamp-2">
                          {largestProspect?.grantor_name ??
                            "No grant range listed"}
                        </span>
                        {largestProspect && <span>→</span>}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="min-h-[210px] rounded-[26px] border border-[#C8CBCC] bg-[#E9E9E7] p-6 shadow-[0_12px_30px_rgba(47,48,56,0.06)]">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <span className="rounded-full border border-[#C8CBCC] bg-white px-2.5 py-1 text-[10px] font-bold text-[#626A70]">
                        {recentlyUpdatedProspects.length}
                      </span>
                    </div>
                    <div className="mt-4 space-y-2">
                      {recentlyUpdatedProspects.map((prospect) => (
                        <button
                          key={prospect.id}
                          type="button"
                          onClick={() =>
                            setSelectedCustomerId(String(prospect.id))
                          }
                          className="group/prospect-recent flex w-full items-center gap-3 rounded-xl border border-transparent bg-white/70 px-3 py-2.5 text-left transition hover:border-[#C8CBCC] hover:bg-white hover:shadow-sm"
                        >
                          ...
                          <span className="text-[#778189] opacity-40 transition group-hover/prospect-recent:opacity-100">
                            →
                          </span>
                        </button>
                      ))}
              ...
              <section className="mb-7">
                <div className="mb-5">
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#626A70]">
                    A quick view of closing deadlines and newly added
                    opportunities before you explore the full directory.
                  </p>
                </div>
                  ...
                  <div className="relative flex min-h-[210px] flex-col overflow-hidden rounded-[26px] border border-[#C8CBCC] bg-white p-6 shadow-[0_14px_36px_rgba(47,48,56,0.08)]">
                    ...
                    <div className="relative flex flex-1 flex-col">
                      ...
                      {upcomingDeadlines.length > 0 ? (
                        <div className="mt-3 flex flex-1 flex-col gap-3">
                          {upcomingDeadlines.slice(0, 3).map((customer) => (
                            <button
                              key={customer.id}
                              type="button"
                              onClick={() =>
        ...
        ================================================== */}
            ...
            ================================================== */}
                ...
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder={
                      isProspectView
                        ? "Search grantors, program areas, categories, key words..."
                        : isArchivedView
                      ...
                      <div className="space-y-2">
                        {sortRules.map((rule, index) => (
                          <div
                            ...
                            draggable
                            onDragStart={(event) =>
                              handleSortDragStart(event, rule.id)
                            }
                            onDragOver={(event) =>
                              handleSortDragOver(event, rule.id)
                            }
                            onDrop={(event) => handleSortDrop(event, rule.id)}
                            onDragEnd={handleSortDragEnd}
                            className={`rounded-xl border p-3 transition-all ${
                              draggedSortId === rule.id
                                ? "border-[#B7655E] bg-[#E7F0F5] opacity-50"
                                : dragOverSortId === rule.id
                          ...
                          >
                            ...
                            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                              ...
                              >
                                {(
                                  Object.keys(SORT_FIELD_LABELS) as SortField[]
                                ).map((field) => {
                                  const usedByAnotherRule = sortRules.some(
                                    (otherRule) =>
                                      otherRule.id !== rule.id &&
                                      otherRule.field === field,
                                  );
                              ...
                              >
                                <option value="asc">
                                  {getSortDirectionLabel(rule.field, "asc")}
                                </option>

                                <option value="desc">
                                  {getSortDirectionLabel(rule.field, "desc")}
                                </option>
                              </select>
                            </div>
                          </div>
                  ...
                  <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-[#626A70]">
                        Options update from your other selections. Choices
                        within a section use OR; sections combine with AND.
                      </p>
                    </div>
                ...
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-[#626A70]">
                        Build custom rules and choose how each one connects to
                        the previous rule.
                      </p>
                    </div>
                  ...
                  {advancedFilters.length === 0 ? (
                    ...
                    <div className="mt-5 space-y-3">
                      {advancedFilters.map((filter, filterIndex) => {
                        const options = getAdvancedFilterOptions(filter.field);

                        const dropdownIsOpen =
                          openAdvancedDropdownId === filter.id;
                        ...
                        return (
                          ...
                          >
                              ...
                              <select
                                ...
                                onChange={(event) =>
                                  updateAdvancedFilter(filter.id, {
                                    connector: event.target.value as
                                      "AND" | "OR",
                                  })
                                }
                            ...
                            <select
                              ...
                              className="rounded-lg border border-[#DFE0E0] bg-white px-3 py-2 text-sm text-[#394147] outline-none focus:border-[#B7655E]"
                            >
                              {getAdvancedOperators(filter.field).map(
                                (operator) => (
                                  <option key={operator} value={operator}>
                                    {getOperatorLabel(operator, filter.field)}
                                  </option>
                                ),
                              )}
                            </select>
                            ...
                            <div
                              ref={dropdownIsOpen ? advancedDropdownRef : null}
                                  ...
                                  {filter.values.map((value) => (
                                    <button
                                      ...
                                      className={`inline-flex max-w-full items-center gap-1 border px-2.5 py-1.5 text-xs font-semibold transition hover:brightness-95 ${
                                        filter.field === "anticipated_deadline"
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2">
                  ...
                  {quickMonths.map((month) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        toggleArrayValue(month, quickMonths, setQuickMonths)
                      }
                  ...
                  {activeAdvancedFilterCount > 0 && (
                    ...
                    >
                      Advanced filters: {activeAdvancedFilterCount} ×
                    </button>
                  )}
          ...
          ================================================== */}
              ...
              {(search.trim() || totalActiveFilterCount > 0) && (
                ...
                <>
                  <div className="space-y-3 p-3 md:hidden">
                    {filteredCustomers.map((prospect) => {
                      const selected =
                        String(selectedCustomerId) === String(prospect.id);
                      const formatGrant = (
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-[20px] border p-4 text-left shadow-sm transition active:scale-[0.995] ${selected ? "border-[#2F3038] bg-[#F2D9D5] shadow-[inset_5px_0_0_#2F3038,0_10px_26px_rgba(47,48,56,0.10)]" : "border-[#D1D3D4] bg-white"}`}
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span className="text-lg text-[#69747C]">→</span>
                          </div>
                          <div className="mt-3 flex flex-wrap gap-2">
                            {(prospect.program_areas ?? []).map((area) => (
                              <span
                                key={area}
                                className="rounded-full border border-[#C8CBCC] bg-[#E9E9E7] px-3 py-1.5 text-xs font-semibold text-[#394147]"
                              >
                                {area}
                              </span>
                            ))}
                          ...
                          <div className="mt-4 grid grid-cols-2 gap-2.5">
                            <div className="rounded-xl border border-[#D7D9DA] bg-[#F4F3F1] p-3">
                              ...
                              <p className="mt-1.5 text-sm font-bold text-[#3E454B]">
                                {formatGrant(prospect.grant_minimum)}
                              </p>
                            </div>
                            <div className="rounded-xl border border-[#D7D9DA] bg-[#F4F3F1] p-3">
                              <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-[#778189]">
                              ...
                              <p className="mt-1.5 text-sm font-bold text-[#3E454B]">
                                {formatGrant(prospect.grant_maximum)}
                              </p>
                            </div>
                          </div>
                          <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <table className="w-full">
                      <thead className="bg-[#2F3038] text-white">
                        <tr>
                          {[
                            ["Grantor", "text-left"],
                            ["Program Areas", "text-left"],
                            ["Minimum Grant", "text-center"],
                            ["Maximum Grant", "text-center"],
                            ["Categories", "text-center"],
                          ].map(([label, alignment]) => (
                            <th
                              key={label}
                              className={`border-b border-white/10 p-5 ${alignment} text-[11px] font-bold uppercase tracking-[0.16em] text-white/75`}
                            >
                              {label}
                            </th>
                          ))}
                        ...
                        {filteredCustomers.map((prospect, index) => {
                          ...
                          return (
                            <tr
                              ...
                              className={`group cursor-pointer transition-all duration-200 ${selected ? "bg-[#F2D9D5] shadow-[inset_6px_0_0_#2F3038,0_8px_24px_rgba(47,48,56,0.10)]" : index % 2 === 0 ? "bg-[#F4F3F1] hover:bg-white" : "bg-[#ECEDEE] hover:bg-white"}`}
                            >
                              <td className="border-l-4 border-transparent p-5 align-top text-left transition-colors group-hover:border-[#C2A05A]">
                                <div className="flex min-w-[170px] items-center gap-3">
                                  ...
                                  <span className="font-semibold leading-5 text-[#3E454B]">
                                    {grantor || "—"}
                                  </span>
                                </div>
                              </td>
                                ...
                                <div className="flex flex-wrap justify-center gap-2">
                                  {(
                                    ...
                                    <button
                                      ...
                                      className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${getCategoryStyle(category, categoryColorMap)} transition hover:-translate-y-0.5 hover:brightness-95 hover:shadow-md`}
                                    >
                                      {category}
                                    </button>
                                  ))}
              ...
              {!isProspectView && (
                <div className="space-y-3 p-3 md:hidden">
                  {filteredCustomers.map((customer) => {
                    ...
                    const daysUntilDeadline = getDeadlineDays(
                      customer.deadline,
                    );
                    const deadlineDate = customer.deadline
                      ? new Date(`${customer.deadline}T00:00:00`)
                      : null;
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <button
                            ...
                            disabled={
                              !favoritesReady ||
                              favoritePendingIds.has(String(customer.id))
                            }
                            onClick={(event) => {
                              event.stopPropagation();
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-2.5">
                          <div className="rounded-xl border border-[#D7D9DA] bg-[#F4F3F1] p-3">
                            ...
                            <p className="mt-1.5 text-sm font-bold text-[#3E454B]">
                              {customer.maximum_grant || "Not specified"}
                            </p>
                          </div>
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          ...
                          {(customer.rfp_categories ?? []).map((category) => (
                            <button
                              key={category}
                              type="button"
                              onClick={(event) => {
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-[#D7D9DA]">
                      {filteredCustomers.map((customer, index) => {
                        ...
                        const isPastDeadline = customer.deadline
                          ? new Date(`${customer.deadline}T00:00:00`) <
                            new Date()
                          : false;
                        ...
                        const grantorInitials = (customer.grantor || "—")
                          .split(/\s+/)
                          .filter(Boolean)
                          .slice(0, 2)
                          .map((word) => word.charAt(0).toUpperCase())
                          ...
                          >
                            ...
                            <td className="p-5 align-top text-center">
                              {customer.anticipated_deadline ? (
                                ...
                                <span className="text-[#7D858B]">—</span>
                              )}
                            </td>
              ...
              {filteredCustomers.length === 0 && (
                <div className="bg-[#E2E3E3] px-6 py-20 text-center">
                  ...
                  <p className="mt-2 text-sm text-[#626A70]">
                    Try adjusting your search or filters to find matching
      ...
      ================================================== */}
            ...
            <CustomerDrawer
              ...
              mode={isProspectView ? "prospect" : "opportunity"}
              isFavorite={
                selectedCustomer
                  ? favoriteIds.has(String(selectedCustomer.id))
                  : false
              }
              favoriteDisabled={
                !favoritesReady ||
                (selectedCustomer
                  ? favoritePendingIds.has(String(selectedCustomer.id))
                  : false)
              }
              onToggleFavorite={
                isProspectView
